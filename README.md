...
- 해당 글 읽기 페이지(read): 하나의 글을 보여주는 페이지
- 예시: http://localhost:3000/community/about

- 이렇게 여러 페이지로 구성된 웹 애플리케이션을 만들 때 페이지 별로 컴포넌트를 분리해가면서
...
- 사용자 인터랙션이 많고 다양한 정보를 제공하는 모던 웹 애플리케이션에 적합하다.
...
- 기존의 페이지에 띄웠던 웹 애플리케이션을 그대로 유지하면서 라우팅 설정에 따라 또다른 페이지를 보여주게 된다.
...
- Link 컴포넌트는 a 태그를 사용하긴 하지만, 페이지를 새로 불러오는 것을 막고 History API를 통해 브라우저 주소의 경로만 바꾸는 기능으로 내장되어 있다.
  ...
  return (
    <div>
      ...
      <ul>
        ...
        <li>
          <Link to="/profiles/sumin">수민의 프로필</Link>
        </li>
        <li>
          <Link to="/profiles/soyeon">소연의 프로필</Link>
        </li>
      </ul>
    </div>
  );
...
const About = () => {
  ...
  // const [쿼리파라미터를 조회/수정하는 메서드들이 담긴 객체반환,
  // 쿼리파라미터를 객체 형태로 업데이트 할 수 있는 함수를 반환] = useSearchParams();
...
- 중첩된 라우트와 Outlet은 페이지끼리 공통적으로 보여줘야하는 레이아웃이 있을 때도 유용하게 사용 가능
...
function App() {
  return (
    <Routes>
      ...
      <Route path="/articles" element={<Articles />}>
        <Route index element={<Navigate replacea to="1" />} />
        <Route path=":id" element={<Article />} />
      </Route>
    </Routes>
  );
...
- 링크에서 사용하는 경로기 현재 라우트의 경로와 일치하는 경우
- 특정 스타일 또는 CSS 클래스를 적용하는 컴포넌트이다.
- 잘 쓰지는 않지만 사용하는 경우가 있다.
