...
- 참고: 컴포넌트의 라이프사이클
  ...
  - 모든 리액트 컴포넌트에는 라이프사이클(생명주기)가 존재
...
- 액션생성함수: 주문서 작성 (아메리카노 하나랑 라떼 하나 주세요)
  - 페이로드 : 주문서 작성 (아메리카노는 _샷추가_, 라떼 우유는 *두유*로 바꿔주세요)
- 디스패치함수 : 주문하기
- 리듀서함수 : 바리스타. 주문 받은 것을 제작/가공해서 손님에게 준다.
  <br>
...
- 리듀서 함수에서 새로운 상태를 만들 때는 반드시 **불변성 유지**를 지켜줘야 한다.
...
const CounterReducer = () => {
  ...
  // dispatch(action)과 같은 형태로, 함수 안에 파라미터로 액션 값을 넣어주면 리듀서 함수가 호출되는 형태이다.
